...
namespace CsharpSyscall
{
    class Program
    {
        public static void Main()
        {
            ...
            res = SysCall.NtAllocateVirtualMemory(hProcess, ref BaseAddress, ZeroBits, ref RegionSize, AllocationType, Protect);
            Console.WriteLine($"NtAllocateVirtualMemory -> {res}");
            ...
            res = SysCall.NtCreateThreadEx(out hThread, DesiredAccess, ObjectAttributes, hProcess, BaseAddress, lpParameter, CreateSuspended, StackZeroBits, SizeOfStackCommit, SizeOfStackReserve, lpBytesBuffer);
            Console.WriteLine($"NtCreateThreadEx -> {res}\nThread Id -> {SysCall.GetThreadId(hThread)}");

            Console.WriteLine(new Win32Exception());

            res = SysCall.NtWaitForSingleObject(hThread, false, 0);
            Console.WriteLine($"NtWaitForSingleObject -> {res}");
        }
    }
    ...
    class SysCall
    {
        ...
        public static Native.NTSTATUS NtCreateThreadEx(out IntPtr hThread, Native.ACCESS_MASK DesiredAccess, IntPtr ObjectAttributes, IntPtr ProcessHandle, IntPtr lpStartAddress, IntPtr lpParameter, bool CreateSuspended, uint StackZeroBits, uint SizeOfStackCommit, uint SizeOfStackReserve, IntPtr lpBytesBuffer)
        {
            // 定义本地API NtCreateThreadEx 的汇编指令。从WinDbg获得
            ...
            unsafe
            {
                // bNtCreateThreadEx的指针
                // fixed关键字防止GC无意间移动内存并更改指针
                fixed (byte* syscallPtr = bNtCreateThreadEx)
                {
                    // 把指针转换为IntPtr类型
                    IntPtr memoryAddress = (IntPtr)syscallPtr;

                    // 把bNtCreateThreadEx汇编指令数组的内存区域权限更改为 执行
                    if (!VirtualProtectEx(Process.GetCurrentProcess().Handle, memoryAddress, (UIntPtr)bNtCreateThreadEx.Length, (uint)Native.AllocationProtect.PAGE_EXECUTE_READWRITE, out uint oldprotect))
                    {
                        throw new Win32Exception();
                    }

                    // 把NtCreateThreadEx的函数指针转换为委托
        ...
        public static Native.NTSTATUS NtAllocateVirtualMemory(IntPtr hProcess, ref IntPtr BaseAddress, IntPtr ZeroBits, ref UIntPtr RegionSize, uint AllocationType, uint Protect)
        {
            // 定义本地API NtAllocateVirtualMemory 的汇编指令。从WinDbg获得
            ...
            unsafe
            {
                ...
                fixed (byte* ptr = bNtAllocateVirtualMemory)
                {
                    ...
                    // 把bNtAllocateVirtualMemory汇编指令数组的内存区域权限更改为 执行
                    if (!VirtualProtectEx(Process.GetCurrentProcess().Handle, memoryAddress, (UIntPtr)bNtAllocateVirtualMemory.Length, (uint)Native.AllocationProtect.PAGE_EXECUTE_READWRITE, out uint oldprotect))
                    {
                        throw new Win32Exception();
                    }

                    // 把NtAllocateVirtualMemory的函数指针转换为委托
        ...
        public static Native.NTSTATUS NtWaitForSingleObject(IntPtr Object, bool Alertable, uint Timeout)
        {
            // 定义本地API NtWaitForSingleObject 的汇编指令。从WinDbg获得
            ...
            unsafe
            {
                // NtWaitForSingleObject的指针
                // fixed关键字防止GC无意间移动内存并更改指针
                fixed (byte* ptr = bNtWaitForSingleObject)
                {
                    // 把指针转换为IntPtr类型
                    IntPtr memoryAddress = (IntPtr)ptr;

                    // 把NtWaitForSingleObject汇编指令数组的内存区域权限更改为 执行
                    if (!VirtualProtectEx(Process.GetCurrentProcess().Handle, memoryAddress, (UIntPtr)bNtWaitForSingleObject.Length, (uint)Native.AllocationProtect.PAGE_EXECUTE_READWRITE, out uint oldprotect))
                    {
                        throw new Win32Exception();
                    }

                    // 把NtWaitForSingleObject的函数指针转换为委托
                    ...
                    return assembledFunction(Object, Alertable, Timeout);
                }
            }
        }
    }
